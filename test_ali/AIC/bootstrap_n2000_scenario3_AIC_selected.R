...
# ============================================================
SCENARIOS      <- c("constant")   # scenario3: manuscript weight-function scenario "constant"
...
TRUE_PAF       <- 0.378045 # exact value from the manuscript's own true-value machinery
                            # (5_function_to_get_summary_plot_and_table.R), for weight_hypo="constant":
                            #   P0_true = 1 - exp(-100*lambda0) = 0.048771  (A=0 throughout, same for all scenarios)
                            #   true_P_observed = 0.078415  (hardcoded table, constant/HR=4/lambda=0.0005)
                            #   true_PAF = (0.078415 - 0.048771) / 0.078415 = 0.378045
                            # Depends only on weight_hypo, lambda0, HRA -- NOT on n, so this value
                            # is unchanged from the manuscript's n=10000/50000 scenarios.
OUT_DIR        <- "output_n2000_scenario3_AIC_selected"
