...
# ============================================================
# ---- fit the 22-model candidate set, record everything worth keeping ----
# ============================================================
fit_and_summarize <- function(data, n, n.visit = 100) {
  fit <- fit_models(dat.complet = data, n = n, n.visit = n.visit)
  ipw_names <- grep("ipw", names(fit), value = TRUE)   # 22 manuscript candidates
  ...
  data.frame(
    ...
    flag_implausible_PAF_AIC = isTRUE(abs(aic_res["PAF"]) > 1),   # sanity flag for the AIC-selected PAF
...
bootstrap_PAF_parallel <- function(d, scenario, sim_id, B, n.visit = 100, n_cores = NULL,
  ...
  cat(sprintf("[%s | sim %d] point estimate PAF (AIC): %.4f | model: %s\n",
              scenario, sim_id, point_res$PAF_AIC, point_res$model_AIC))
  ...
  cat("scenario\tsim_id\tboot_id\ttime_stamp\tPAF_AIC\tmodel_AIC\telapsed_min\n", file = prog)
  ...
  boot_rows <- foreach(b = 1:B,
    ...
    gc()  # garbage collect at start of each replicate (cluster memory hygiene --
          # more important here than in the n=1000 runs, since each worker's
          # boot_data is proportionally larger at n=2000)
    ...
    paf_str   <- if (!is.null(out$row)) round(out$row$PAF_AIC, 4) else NA
    model_str <- if (!is.null(out$row)) out$row$model_AIC else paste0("FAILED: ", out$error)
    cat(sprintf("%s\t%d\t%d\t%s\t%s\t%s\t%.1f\n",
                scenario, sim_id, b, format(Sys.time(), "%H:%M:%S"), paf_str, model_str,
  ...
  paf <- boot_results$PAF_AIC
  cat(sprintf("  -> %s sim %d: bootstrap SE=%.4f | mean PAF=%.4f | 95%%CI [%.4f, %.4f] | events=%.0f | failed=%d/%d\n\n",
              scenario, sim_id, sd(paf), mean(paf), quantile(paf, .025), quantile(paf, .975),
...
# ============================================================
SCENARIOS      <- c("only_current")   # scenario4: manuscript weight-function scenario "only_current"
...
TRUE_PAF       <- 0.551951 # exact value from the manuscript's own true-value machinery
                            # (5_function_to_get_summary_plot_and_table.R), for weight_hypo="only_current":
                            #   P0_true = 1 - exp(-100*lambda0) = 0.048771  (A=0 throughout, same for all scenarios)
                            #   true_P_observed = 0.108851  (hardcoded table, only_current/HR=4/lambda=0.0005)
                            #   true_PAF = (0.108851 - 0.048771) / 0.108851 = 0.551951
                            # Depends only on weight_hypo, lambda0, HRA -- NOT on n, so this value
                            # is unchanged from the manuscript's n=10000/50000 scenarios.
OUT_DIR        <- "output_n2000_scenario4_AIC_selected"

cat("Starting n=1000 bootstrap-vs-model-selection study (all 4 scenarios)...\n")
...
# One row per scenario, aggregating across all outer datasets.
#
# The key quantities for answering the reviewers:
#   empirical_SE  = SD of the point estimates across the outer datasets.
#                   This IS the ground-truth sampling variability of the
#                   AIC-selected PAF, so it needs no separate run.
#   mean_boot_SE  = average of the within-dataset bootstrap SEs.
#   SE_ratio      = mean_boot_SE / empirical_SE   -> should be ~1 if the
#                   bootstrap estimates the variance correctly.
#   coverage      = % of outer datasets whose bootstrap 95% percentile CI
#                   contains the true PAF (only computed if TRUE_PAF is set).
# ============================================================
all_files <- list.files(OUT_DIR, pattern = "^bootstrap_.*_results\\.rds$", full.names = TRUE)
all_data  <- lapply(all_files, readRDS)

# ---- per-dataset table: one row per outer dataset ----
per_dataset <- do.call(rbind, lapply(all_data, function(x) {
  bp <- x$all_results$PAF_AIC[x$all_results$type == "bootstrap"]
  data.frame(
    scenario     = x$scenario,
    sim_id       = x$sim_id,
    point_PAF    = x$point$PAF_AIC,
    point_model  = x$point$model_AIC,
    boot_SE      = sd(bp),
    boot_mean    = mean(bp),
    ci_lower     = unname(quantile(bp, 0.025)),
    ci_upper     = unname(quantile(bp, 0.975)),
...
write.csv(per_dataset, file.path(OUT_DIR, "per_dataset_summary.csv"), row.names = FALSE)
...
summary_rows <- lapply(unique(per_dataset$scenario), function(scen) {
  ...
  boot_all  <- do.call(rbind, lapply(scen_data,
                 function(x) x$all_results[x$all_results$type == "bootstrap", ]))

  emp_SE       <- sd(pd$point_PAF)                 # ground truth sampling SD
  mean_boot_SE <- mean(pd$boot_SE)
  n_out        <- nrow(pd)

  # Monte Carlo SE of the coverage estimate (Morris et al. 2019)
  cov_pct <- NA_real_; cov_mcse <- NA_real_; bias <- NA_real_
  if (!is.na(TRUE_PAF)) {
    covered  <- (pd$ci_lower <= TRUE_PAF) & (TRUE_PAF <= pd$ci_upper)
    cov_pct  <- 100 * mean(covered)
    cov_mcse <- 100 * sqrt((cov_pct/100) * (1 - cov_pct/100) / n_out)
    bias     <- mean(pd$point_PAF) - TRUE_PAF
  }
  ...
  data.frame(
    ...
    true_PAF                = TRUE_PAF,
    mean_point_PAF          = mean(pd$point_PAF),
    bias                    = bias,
    bias_MCSE               = sd(pd$point_PAF) / sqrt(n_out),
    empirical_SE            = emp_SE,
    mean_bootstrap_SE       = mean_boot_SE,
    SE_ratio                = mean_boot_SE / emp_SE,
    sd_bootstrap_SE         = sd(pd$boot_SE),
    coverage_pct            = cov_pct,
    coverage_MCSE           = cov_mcse,
    mean_CI_width           = mean(pd$ci_upper - pd$ci_lower),
    pct_unstable_flag       = 100 * mean(boot_all$flag_unstable_AIC, na.rm = TRUE),
    pct_implausible_PAF     = 100 * mean(boot_all$flag_implausible_PAF_AIC, na.rm = TRUE),
    pct_BIC_AIC_agree       = 100 * mean(boot_all$same_model_selected, na.rm = TRUE),
    n_unique_models         = length(unique(boot_all$model_AIC)),
    modal_model             = names(sort(table(boot_all$model_AIC), decreasing = TRUE))[1],
    modal_model_pct         = 100 * max(table(boot_all$model_AIC)) / nrow(boot_all),
...
cat("\nModel selection frequency (pooled across all bootstrap replicates):\n")
boot_pool <- do.call(rbind, lapply(all_data,
                function(x) x$all_results[x$all_results$type == "bootstrap", ]))
print(sort(table(boot_pool$model_AIC), decreasing = TRUE))

cat("\nModel selected on the ORIGINAL datasets (point estimates):\n")
print(sort(table(per_dataset$point_model), decreasing = TRUE))

write.csv(summary_table, file.path(OUT_DIR, "manuscript_summary_table.csv"), row.names = FALSE)
saveRDS(list(summary_table = summary_table, per_dataset = per_dataset, all_data = all_data),
        file.path(OUT_DIR, "manuscript_summary_table.rds"))
cat("\nSaved manuscript_summary_table.csv, per_dataset_summary.csv and .rds in", OUT_DIR, "\n")
