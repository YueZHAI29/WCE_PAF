...
# ============================================================
# bootstrap_PAF_parallel: parallel inner loop for ONE dataset
# ============================================================
bootstrap_PAF_parallel <- function(d, sim_id = 1, B = 500, n.visit = 100, n_cores = NULL,
                                   out_dir = "test_output") {

  if (is.null(n_cores)) {
    n_cores <- min(parallel::detectCores() - 1, B)
  }
  ...
  cat("[sim", sim_id, "] point estimate PAF (BIC):", round(point_res$PAF_BIC, 4),
      "| model:", point_res$model_BIC, "\n")

  # pre-draw all bootstrap ids up front -- reproducible, independent of
  ...
  # Only export FUNCTION NAMES here. Most of fit_models()'s helpers
  # (compute_ipw, .predict_P0_ipw, .predict_P1_ipw) are RE-DEFINED locally
  # inside fit_models()'s own body, so they travel with it automatically.
  # .ipw_wce is the ONE exception: it exists only as a top-level function,
  # and fit_models() reaches it via lexical scoping to its enclosing
  # (global) environment. On a fresh worker process that enclosing
  # environment is the WORKER's own empty .GlobalEnv, not the master's --
  # so .ipw_wce must be exported explicitly, or every WCE-model fit
  # inside fit_models() fails with "could not find function '.ipw_wce'".
  clusterExport(cl, c("sim_MSM_WCE", "fit_models", ".ipw_wce",
                      "resample_subjects", "fit_and_summarize"))

  prog <- file.path(out_dir, sprintf("bootstrap_sim%03d_B%d_progress.log", sim_id, B))
  cat("sim_id\tboot_id\ttime_stamp\tPAF_BIC\tmodel_BIC\telapsed_min\n", file = prog)

  t_start <- Sys.time()
  cat("Starting parallel bootstrap (B =", B, ")...\n")
  ...
  boot_rows <- foreach(b = 1:B,
    ...
    out <- tryCatch({
      ...
      list(row = NULL, error = conditionMessage(e))
    })

    paf_str   <- if (!is.null(out$row)) round(out$row$PAF_BIC, 4) else NA
    model_str <- if (!is.null(out$row)) out$row$model_BIC else paste0("FAILED: ", out$error)
    cat(sprintf("%d\t%d\t%s\t%s\t%s\t%.1f\n",
                sim_id, b, format(Sys.time(), "%H:%M:%S"), paf_str, model_str,
  ...
  cat("Parallel bootstrap completed in", round(elapsed, 1), "minutes (", n_cores, "cores )\n")

  ok       <- sapply(boot_rows, function(x) is.null(x$error) || is.na(x$error))
  n_failed <- sum(!ok)
  if (n_failed > 0) {
    cat("WARNING:", n_failed, "of", B, "bootstrap replicates failed\n")
    cat("---- error messages (first 5 shown) ----\n")
    err_msgs <- sapply(boot_rows[!ok], function(x) x$error)
    print(head(err_msgs, 5))
    cat("-----------------------------------------\n")
  }
  boot_results <- do.call(rbind, lapply(boot_rows[ok], function(x) x$row))
  all_results  <- rbind(point_res, boot_results)

  out_rds <- file.path(out_dir, sprintf("bootstrap_sim%03d_B%d_results.rds", sim_id, B))
  out_csv <- file.path(out_dir, sprintf("bootstrap_sim%03d_B%d_results.csv", sim_id, B))
  saveRDS(list(sim_id = sim_id, all_results = all_results, point = point_res,
  ...
  paf <- boot_results$PAF_BIC
  cat("\n--- sim", sim_id, "summary ---\n")
  cat("Bootstrap mean PAF :", round(mean(paf), 4), "\n")
  cat("Bootstrap SE       :", round(sd(paf), 4), "\n")
  cat("Bootstrap 95% CI   :", round(quantile(paf, c(.025, .975)), 4), "\n")

  return(list(all_results = all_results, point = point_res, elapsed_min = elapsed, n_failed = n_failed))
}

# ============================================================
# Run: one scenario, n=1000, decreasing / HRA=4 / lambda0=0.0005
# ============================================================
cat("Starting bootstrap study...\n")
start_time <- Sys.time()

# ============================================================
# ---- CONFIG: change ONLY these three lines to scale up ----
# For the colleague's first test run, keep these small (same total
# workload as testing B=500 once, but also exercises the outer loop):
OUTER_SIM_IDS <- 1:2     # test: 5 outer datasets  (final: e.g. 1:500)
INNER_B       <- 500     # test: 100 inner bootstraps per dataset (final: 500)
N_CORES       <- 32      # cluster will cap at detectCores()-1 if smaller
# ============================================================

cat(sprintf("Plan: %d outer datasets x %d inner bootstraps = %d total fits (+ %d point estimates)\n",
            length(OUTER_SIM_IDS), INNER_B, length(OUTER_SIM_IDS) * INNER_B, length(OUTER_SIM_IDS)))

for (sim_id in OUTER_SIM_IDS) {

  out_rds_check <- file.path("test_output", sprintf("bootstrap_sim%03d_B%d_results.rds", sim_id, INNER_B))
  if (file.exists(out_rds_check)) {
    cat(sprintf(">> sim_id=%d already done (%s) -- skipping\n", sim_id, out_rds_check))
    next
  }
  ...
  set.seed(sim_id)
  d <- sim_MSM_WCE(weight_hypo = "decreasing", true_cutoff = 50, n = 1000,
                   n.visit = 100, lambda0 = 0.0005, HRA = 4, T0toL = 1,
                   seed = sim_id)$dat.complet
  cat("Event rate:", mean(tapply(d$event, d$id, max)), "\n")

  result <- bootstrap_PAF_parallel(d, sim_id = sim_id, B = INNER_B, n.visit = 100, n_cores = N_CORES)
}

end_time <- Sys.time()
total_min <- as.numeric(difftime(end_time, start_time, units = "mins"))
cat("\nTotal execution time:", round(total_min, 2), "minutes for",
    length(OUTER_SIM_IDS), "outer x", INNER_B, "inner\n")
cat(sprintf("Extrapolated to 500 outer x 500 inner: ~%.1f hours\n",
            total_min * (500/length(OUTER_SIM_IDS)) * (500/INNER_B) / 60))
